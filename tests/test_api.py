...
def test_exposed_objects(modulelevel_internal_objects: list[griffe.Object | griffe.Alias]) -> None:
    """All public objects in the internal API are exposed under `dm_aio_mcp`."""
    not_exposed = [
        obj.path
        for obj in modulelevel_internal_objects
        if obj.name not in dm_aio_mcp.__all__ or not hasattr(dm_aio_mcp, obj.name)
    ]
...
def test_single_locations(public_api: griffe.Module) -> None:
    ...
    multiple_locations = {}
    for obj_name in dm_aio_mcp.__all__:
...
def test_inventory_matches_api(
    ...
    public_api_paths.add("dm_aio_mcp")
    for item in inventory.values():
        if (
            item.domain == "py"
            and "(" not in item.name
            and (item.name == "dm_aio_mcp" or item.name.startswith("dm_aio_mcp."))
        ):
